...
load("MSPverWHIM.rda")
load("MSPverWHIMout3.rda")

# Valores de tendencia de la especie
trend.log.samples <- MCMCchains(out3$beta.samples, params = 'scale(year)', exact = TRUE)

n.years <- n_distinct(MSPverWHIM$abund.covs[, 'year'])

# Valores de intercepto
int.samples <- MCMCchains(out3$beta.samples, params = '(Intercept)', exact = TRUE)

# Efectos aleatorios (sitio y unidad anidada)
trend.re.samples <- MCMCchains(out3$beta.star.samples, params = c('sitio', 'SUID'), exact = FALSE)
...
pred.df$year.s <- (2017:2023 - mean(MSPverWHIM$abund.covs[, 'year'], na.rm = TRUE)) / sd(MSPverWHIM$abund.covs[, 'year'], 
                                                                                                                                                                                  na.rm = TRUE)
...
slope_samples <- MCMCchains(out3$beta.samples, params = 'scale(year)', exact = TRUE)
...
ggplot(data = plot.df, aes(x = year, y = med)) + 
  ...
  labs(x = '', y = 'Índice') +
  annotate("text", x = 2019.5, y = 360, label = "italic('Numenius phaeopus')", size = 6, hjust = 0.5, parse = TRUE) +
  annotate("text", x = 2019.5, y = 340, label = "Temporada Verano", size = 4, hjust = 0.5)
